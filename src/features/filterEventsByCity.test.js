...
defineFeature(feature, test => {
  ...
  test('When user hasn\'t searched for a city, show upcoming events based on the user\'s location by default', ({ given, when, then })=> {
  ...
  test('User can select a city from the suggested list', ({ given, and, when, then }) => {
    ...
    given('user is typing "Manchester" in the city textbox', () => {
      AppWrapper = mount(<App />);
      AppWrapper.find('.city').simulate('change', { target: { value: 'Manchester' } });
    });
